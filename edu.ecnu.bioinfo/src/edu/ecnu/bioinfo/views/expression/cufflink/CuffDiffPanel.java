...
public class CuffDiffPanel extends Composite {
	public CuffDiffPanel(Composite parent, int style) {
		super(parent, style);
	...
	private boolean checkDiffParameters() {
		isOk = true;
	...
	public String[] getDiffParameters() {
		String[] DiffParameters = new String[25];
		for (int i = 0; i < DiffParameters.length; i++)
			DiffParameters[i] = " ";
		if (!checkDiffParameters()) {
			return DiffParameters;
		}
		file = textFile.getText();
		//creatDiff
		DiffParameters[0] = file;
		numth = spinnerNumth.getSelection();
		DiffParameters[1] = "" + numth;
		seedN = spinnerSeedN.getSelection();
		DiffParameters[2] = "" + seedN;
		minAli = spinnerMinAli.getSelection();
		DiffParameters[3] = "" + minAli;
		
		fDR = textFDR.getText();
		DiffParameters[4] = "" + fDR;	
		if (buttonMaskF.getSelection()) {
			DiffParameters[5] = "" + "-M";
		}
		tim = buttonTime.getSelection();
		DiffParameters[6] = "" + tim;
		if (buttonFragB.getSelection()) {
			DiffParameters[7] = "" + "-b";
		}
		mulR = buttonMulR.getSelection();
		DiffParameters[8] = "" + mulR;
		upp= buttonUpp.getSelection();
		DiffParameters[9] = ""+upp;
		lab = buttonLabel.getSelection();
		DiffParameters[10] = ""+lab;
		
		
		//creatAdvance
		fraM = spinnerFraM.getSelection();
		DiffParameters[11] = "" + fraM;
		fraS = spinnerFraS.getSelection();
		DiffParameters[12] = "" + fraS;
		numIm = spinnerNumIm.getSelection();
		DiffParameters[13] = "" + numIm;
		numBoot = spinnerNumBoot.getSelection();
		DiffParameters[14] = "" + numBoot;
		boots = textBoots.getText();
		DiffParameters[15] = "" + boots;
		maxMle = spinnerMaxMle.getSelection();
		DiffParameters[16] = "" + maxMle;
		maxBu = spinnerMaxBu.getSelection();
		DiffParameters[17] = "" + maxBu;
		cnorm = buttonCnorm.getSelection();
		DiffParameters[18] = "" + cnorm;
		tnorm = buttonTnorm.getSelection();
		DiffParameters[19] = ""+ tnorm;
		poiss = buttonPoiss.getSelection();
		DiffParameters[20] = "" + poiss;
		ver = buttonVer.getSelection();
		DiffParameters[21] ="" + ver;
		qui = buttonQui.getSelection();
		DiffParameters[22] = ""+qui;
		nudate= buttonNUpdate.getSelection() ;
		DiffParameters[23] = "" + nudate;
		emit = buttonEmit.getSelection();
		DiffParameters[24] = ""+ emit;	
